...
-- literals). For any line that /starts/ inside an open block comment
-- we demote a leading @Important \"#\"@ token to @Other \"#\"@. That
-- keeps the @#@ character in the output byte-for-byte (detokenize
-- concatenates all tokens indiscriminately) while making the
-- directive detector in "Hpp.Directive" skip it.
--
-- We deliberately do /not/ rewrite the text of comments, and we do
-- /not/ suppress macro expansion inside them — the only change is
-- that a stray leading @#@ sitting inside a block comment no longer
-- trips directive dispatch.
...
gateHaskellComments = go HsCode
  where
    go _ []         = []
    go st (ln:lns)  =
      let startedInBlockCmt = case st of HsBlockCmt _ -> True; _ -> False
          ln'  | startedInBlockCmt = demoteLeadingHash ln
               | otherwise         = ln
  ...
  do cfg <- getL config <$> getState
     ...
     case () of
       _ | eraseCComments cfg && spliceLongLines cfg
           && not (inhibitLinemarkers cfg) -> pure (gate . normalCPP)
       _ | (eraseCComments cfg && spliceLongLines cfg
            && (not (replaceTrigraphs cfg))) ->
           pure (gate . haskellCPP)
       _ | not (any ($ cfg) [ eraseCComments
                            , spliceLongLines
                            , replaceTrigraphs ]) -> pure (gate . onlyMacrosCPP)
       _ | otherwise -> pure (gate . genericConfig cfg)
